...
export async function getMedicationHistoryByUserID(userId: string): Promise<History> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Error fetching medication history: ${response.statusText}`);
      }
  ...
  export async function postMedicationTransaction(requestData: CheckoutMedicationRequest): Promise<{ message: string; payment_url?: string; }> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Error posting medication transaction: ${response.statusText}`);
      }
